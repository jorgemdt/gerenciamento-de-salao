...
if __name__ == '__main__':
    ...
    # Cria uma janela Tkinter raiz para servir como 'master'
    root = tk.Tk()
    root.withdraw() # Esconde a janela raiz, só queremos ver o Toplevel

    # Cria um 'Mock Controller' - um objeto simples que tem o método
    # esperado pela View, mas que só imprime no console ou mostra um popup.
    class MockControllerForTest:
        def __init__(self, root_window):
            self.root = root_window
            self.view = None # Guardará a referência da view

        def set_view(self, view):
            self.view = view

        def handle_login_event(self, event=None):
            # Pega o usuário da view (se ela foi setada)
            username = self.view.get_username() if self.view else "N/A"
            print(f"Botão 'Entrar' clicado! Usuário: {username}")
            # Mostra um popup de teste
            messagebox.showinfo("Teste de Login",
                                f"Tentativa de login com: '{username}'",
                                parent=self.view.window)
            # Fecha a aplicação de teste após o clique
            self.root.destroy()

    # Instancia o mock controller
    mock_controller = MockControllerForTest(root)

    # Instancia a LoginView, passando a raiz e o mock controller
    login_view = LoginView(root, mock_controller)

    # Dá ao mock controller a referência da view para que ele possa interagir
    mock_controller.set_view(login_view)

    # Define o que fazer ao fechar a janela no modo de teste (fechar tudo)
    login_view.window.protocol("WM_DELETE_WINDOW", root.destroy)

    # Garante que a janela de login seja mostrada (sem o grab_set/wait_window)
    login_view.window.deiconify()

    root.mainloop()

    print("Janela de teste fechada.")